...
This is a guided code-reading path for `sparse-count-pca`. It emphasizes
...
- [`_counts.py`](../src/sparse_count_pca/_counts.py): count canonicalization and
...
- [`tests/test_residual_anndata.py`](../tests/test_residual_anndata.py)
- [`tests/test_residual_scanpy.py`](../tests/test_residual_scanpy.py)
- mask-order tests in [`tests/test_log_pca.py`](../tests/test_log_pca.py) and
  [`tests/test_dirichlet.py`](../tests/test_dirichlet.py)
...
[`_residuals.py`](../src/sparse_count_pca/_residuals.py) contains Poisson,
...
[`_clip.py`](../src/sparse_count_pca/_clip.py) applies exact clipping. Symmetric
...
[`_log_transforms.py`](../src/sparse_count_pca/_log_transforms.py) contains the
...
[`_log_pca.py`](../src/sparse_count_pca/_log_pca.py) provides the fixed-count and
fixed-composition PCA APIs.
[`_dirichlet_pca.py`](../src/sparse_count_pca/_dirichlet_pca.py) provides the
...
Read [`_representation.py`](../src/sparse_count_pca/_representation.py).
...
- [`tests/test_operator.py`](../tests/test_operator.py), especially ranks 0, 1,
  and 3;
- transform materialization tests in
  [`tests/test_log_pca.py`](../tests/test_log_pca.py),
  [`tests/test_dirichlet.py`](../tests/test_dirichlet.py), and
  [`tests/test_deviance.py`](../tests/test_deviance.py).

## 5. Linear operator and numerical statistics

[`_operator.py`](../src/sparse_count_pca/_operator.py) turns the representation
...
[`tests/test_residual_pca.py`](../tests/test_residual_pca.py).

## 6. Shared PCA execution

[`_pca.py`](../src/sparse_count_pca/_pca.py) is the transform-independent PCA
...
[`_svd.py`](../src/sparse_count_pca/_svd.py) owns the narrow ARPACK contract:
...
[`tests/corral_reference/`](../tests/corral_reference/) and
[`tests/test_correspondence.py`](../tests/test_correspondence.py).
...
- [`tests/townes_reference/`](../tests/townes_reference/): residual PCA;
- [`tests/corral_reference/`](../tests/corral_reference/): correspondence
  analysis;
...
The detailed wishlist is in [wishlist.md](wishlist.md). The highest-priority
architectural addition is block-wise reconstruction through retained PCs and,
where mathematically identifiable, back to approximate count space.

That feature should reconstruct transformed blocks as

```text
scores_block @ components + mean
```

then apply a transform-specific inverse using stored or supplied normalization
state. Truncation can produce negative reconstructed counts; the package should
preserve them and leave clipping policy to consumers.
